...
const passwordInput=(e)=>{
  ...
  {!user && (
    <button className='text-white rounded-full px-12 py-2 mx-auto my-5 drop-shadow bg-gradient-to-r from-purple-500 via-purple-600 to-purple-700 font-semibold hover:bg-gradient-to-br ease-in-out transition duration-200' type="submit" disabled={isLoading}>Sign Up</button>
  )}
  {user && (
    <Link to='/detect-location' className='text-white rounded-full px-12 py-2 text-center my-5 mx-5 drop-shadow bg-gradient-to-r from-purple-500 via-purple-600 to-purple-700 font-semibold hover:bg-gradient-to-br ease-in-out transition duration-200' type="submit">Begin Shopping</Link>
  )}
  {error && <div className='error'>{error}</div>}
  {!user && (
    <h6 className='font-normal text-sm pb-9 pt-2'>Already a user? <Link className='text-blue-600' to='/login'>Login</Link></h6>
  )}
  {user && (
    <h6 className='font-normal text-sm pb-9 pt-2'>Sign Up Successful!</h6>
  )}
  </label>
</form>
    </div>
